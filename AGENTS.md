...
> **Purpose:** Working notes for AI assistants working on this repository. This file replaces older Jules-specific review instructions and contains the canonical project-wide guidance. For detailed/situational guidance, see the files listed in "When to read additional guidance" below.
...
1. Read `CLAUDE.md` for Claude Code-specific notes; that file natively includes `@AGENTS.md` for the generic guidance below.
...
```bash
./server_setup.sh              # one-time setup (see docs/agents/commands.md#setup)
source mcp_env/bin/activate    # activate venv
make install-dev               # install dev dependencies
make test-fast                 # preferred test run
make check                     # format + lint + type checks
make run                       # run MCP server
make build                     # build distributions
make clean                     # clean cache and build artifacts
```

> For the full command reference (testing variants, code quality, server transports, debugging, distribution, maintenance, shortcuts) read `docs/agents/commands.md`.
...
## Important Notes for Agents (summary)

- Prefer cpp-analyzer MCP tools over grep/glob when the server is running.
- MCP tools return `brief`/`doc_comment`, method indicators (`is_virtual`, `is_pure_virtual`, etc.), and support flexible pattern matching.
- Incremental analysis is automatic; use `refresh_mode="full"` only after major config changes or suspected cache corruption.
- Project identity is config-based; no automatic discovery of `.cpp-analyzer-config.json`.
- Restart the analyzer after modifying `compile_commands.json`.
- Multi-process workers run in isolated `spawn` processes.
- SQLite cache lives in `.mcp_cache/`; safe to delete for fresh indexing.
- Schema changes auto-recreate the database in development mode.
- Parse error recovery extracts partial symbols from files with non-fatal errors.

> For the full version of these notes with examples and detailed behavior read `docs/agents/behavior.md`.

## When to Read Additional Guidance

| Situation | File to read |
|-----------|--------------|
| Setting up the repository for the first time or after `make clean-all` | `docs/agents/commands.md#setup` |
| Need a full command reference (all test targets, transports, shortcuts, maintenance) | `docs/agents/commands.md` |
| Before architectural changes or deep onboarding into the codebase | `docs/agents/architecture.md` |
| Configuring the analyzer or overriding environment variables | `docs/agents/configuration.md` |
| Debugging performance, file descriptor leaks, or parse errors | `docs/agents/diagnostics.md` |
| Adding a new MCP tool or modifying symbol extraction logic | `docs/agents/workflows.md` |
| Need the complete expanded version of the notes above | `docs/agents/behavior.md` |
