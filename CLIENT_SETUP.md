...
2. The MCP server is working (test with `python -m clang_index_mcp`)
...
2. Start the server separately: `python -m clang_index_mcp --transport http --port 8000`
...
- **args**: Arguments to pass to Python
  - `["-m", "clang_index_mcp"]` - runs the module

- **cwd**: Working directory for the server
  - Must be the root of the clang_index_mcp repository

- **env**: Environment variables
  - `PYTHONPATH`: Ensures Python can find the clang_index_mcp module
...
python -m clang_index_mcp --transport http --port 8000

# Start on custom port
python -m clang_index_mcp --transport http --host 127.0.0.1 --port 9000

# Allow external connections (use with caution)
python -m clang_index_mcp --transport http --host 0.0.0.0 --port 8000
```
...
python -m clang_index_mcp --transport sse --port 8080

# Start on custom port
python -m clang_index_mcp --transport sse --host 127.0.0.1 --port 9000
```
...
ExecStart=/path/to/clang_index_mcp/mcp_env/bin/python -m clang_index_mcp --transport http --host 127.0.0.1 --port 8000
Restart=on-failure
...
# Run server
CMD ["python", "-m", "clang_index_mcp", "--transport", "http", "--host", "0.0.0.0", "--port", "8000"]
```
