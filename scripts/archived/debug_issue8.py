...
def debug_issue8():
    ...
    print(f"\n[Before indexing]")
    print(
        f"  header_tracker.get_processed_count(): {analyzer.header_tracker.get_processed_count()}"
    )
    ...
    if analyzer.header_tracker.get_processed_count() > 0 and len(headers) == 0:
        ...
        print(
            f"  This means headers were claimed but symbols weren't extracted or added to file_index"
        )
        return False
