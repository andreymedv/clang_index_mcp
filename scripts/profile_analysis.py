...
class ProfiledAnalyzer(CppAnalyzer):
    ...
    def index_file(self, file_path: str, force: bool = False):
        ...
        try:
            ...
            if error_diagnostics:
                formatted_errors = BaseAnalyzer._format_diagnostics(
                    self, error_diagnostics, max_count=5
                )
                full_error_msg = (
    ...
    def print_timing_report(self):
        ...
        if success_timings:
            ...
            avg_parse = (
                sum(self.timings.get("libclang_parse", []))
                / len(self.timings.get("libclang_parse", []))
                if self.timings.get("libclang_parse")
                else 0
            )
            avg_traversal = (
                sum(self.timings.get("ast_traversal", []))
                / len(self.timings.get("ast_traversal", []))
                if self.timings.get("ast_traversal")
                else 0
            )
            avg_bulk_write = (
                sum(self.timings.get("bulk_write", [])) / len(self.timings.get("bulk_write", []))
                if self.timings.get("bulk_write")
                else 0
            )
            avg_lock_wait = (
                sum(self.timings.get("lock_wait", [])) / len(self.timings.get("lock_wait", []))
                if self.timings.get("lock_wait")
                else 0
            )
