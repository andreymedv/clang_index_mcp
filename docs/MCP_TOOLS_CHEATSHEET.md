...
```yaml
- name: TextComposer                        # Simple class name
  qualified_name: app::doc::TextComposer    # Full name with namespaces
...
- name: parseDocument                      # Function name
  ...
  signature: "Item (const string &)"   # Parameters and return type
...
namespace: parser
signature: "Item (const string &, ParseOptions)"
return_type: Item                      # Extracted from signature
...
is_template: false
template_parameters: null
brief: "Parse JSON string into Item"
```
