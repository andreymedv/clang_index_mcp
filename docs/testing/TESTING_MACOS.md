...
1. Tests import `clang_index_mcp._mcp.cpp_mcp_server` which triggers module-level libclang initialization
...
3. **Check which libclang is being used**:
   ```bash
   python3 -m clang_index_mcp --help 2>&1 | grep libclang
   ```
