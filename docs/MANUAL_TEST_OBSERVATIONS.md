...
1. Start indexing a large project (e.g., CloudOffice with ~5700+ files)
...
A small LLM model (Qwen3-4B) invoked `search_classes` with `name='SomeName'` and `file='PFX_SomeName.h'` in response to the user question: "what classes are defined in PFX_SomeName.h?". The model appears to be assuming that:
- The file name is related to the class name
- It should search for class "SomeName" within file "PFX_SomeName.h"

**Correct approach should be:**
- `search_classes(pattern='.*', file_name='PFX_SomeName.h')` - list ALL classes in that file
- OR `find_in_file(file_path='PFX_SomeName.h', pattern='.*')` - list ALL symbols in that file
...
```
The example `'MyClass.h'` subtly implies that header files are typically named after the classes they contain. Small models may infer: `PFX_SomeName.h` → contains class `SomeName` or `PFX_SomeName`.
...
```
[WARNING] /path/to/project/Printing/PE_PresentationPrinter.h: Continuing despite 1 error(s):
libclang parsing errors (1 total):
