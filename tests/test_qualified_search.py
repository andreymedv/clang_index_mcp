...
class TestPartiallyQualifiedNameLookups:
    ...
    which caused "ItemBuilder::SlideBuilder" to fail when the actual
    qualified name was "Outer::ItemBuilder::SlideBuilder".
...
""")
            ...
            result = analyzer.get_class_info("Outer::ItemBuilder::SlideBuilder")
            assert result is not None
            assert result["name"] == "SlideBuilder"
            assert result["qualified_name"] == "Outer::ItemBuilder::SlideBuilder"

            # Partially qualified (missing Outer::) should also work
            result = analyzer.get_class_info("ItemBuilder::SlideBuilder")
            assert result is not None
            assert result["name"] == "SlideBuilder"
            assert result["qualified_name"] == "Outer::ItemBuilder::SlideBuilder"

            # Simple name should work
            result = analyzer.get_class_info("SlideBuilder")
            assert result is not None
            assert result["name"] == "SlideBuilder"
