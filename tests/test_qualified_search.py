...
class TestPatternTypeDetection:
    ...
    def test_exact_pattern_leading_colons(self):
        ...
        assert detect_pattern_type("::View") == "exact"
        assert detect_pattern_type("::GlobalClass") == "exact"
        assert detect_pattern_type("::std::string") == "exact"

    def test_unqualified_pattern_no_colons(self):
        """No :: should be detected as unqualified match."""
        assert detect_pattern_type("View") == "unqualified"
        assert detect_pattern_type("string") == "unqualified"
        assert detect_pattern_type("MyClass") == "unqualified"

    def test_suffix_pattern_with_colons(self):
        """:: without leading :: should be detected as suffix match."""
        assert detect_pattern_type("ui::View") == "suffix"
        assert detect_pattern_type("app::core::Config") == "suffix"
        assert detect_pattern_type("ns1::ns2::Class") == "suffix"

    def test_regex_pattern_with_metacharacters(self):
        """Regex metacharacters should be detected as regex."""
        assert detect_pattern_type("app::.*::View") == "regex"
        assert detect_pattern_type(".*View.*") == "regex"
        assert detect_pattern_type("View.*") == "regex"
        assert detect_pattern_type("View+") == "regex"
        assert detect_pattern_type("View?") == "regex"
        assert detect_pattern_type("View[A-Z]") == "regex"
        assert detect_pattern_type("(View|Model)") == "regex"
        assert detect_pattern_type("^View$") == "regex"
        assert detect_pattern_type("View\\w+") == "regex"

    def test_empty_pattern(self):
        """Empty pattern should be unqualified."""
        assert detect_pattern_type("") == "unqualified"
    ...
    def test_regex_takes_precedence_over_suffix(self):
        ...
        assert detect_pattern_type("ns::View.*") == "regex"
        assert detect_pattern_type("ns::(View|Model)") == "regex"
        assert detect_pattern_type("ns::.*") == "regex"
...
class TestQualifiedPatternMatching:
    ...
    def test_exact_match_with_leading_colons(self):
        ...
        assert matches_qualified_pattern("View", "::View") is True
        assert matches_qualified_pattern("GlobalClass", "::GlobalClass") is True

        # Should NOT match namespaced symbols
        assert matches_qualified_pattern("app::View", "::View") is False
        assert matches_qualified_pattern("ns1::ns2::View", "::View") is False
    ...
    def test_unqualified_pattern_matches_any_namespace(self):
        ...
        assert matches_qualified_pattern("View", "View") is True
        assert matches_qualified_pattern("app::View", "View") is True
        assert matches_qualified_pattern("app::ui::View", "View") is True
        assert matches_qualified_pattern("ns1::ns2::ns3::View", "View") is True

        # Should NOT match different names
        assert matches_qualified_pattern("ViewManager", "View") is False
        assert matches_qualified_pattern("app::ListView", "View") is False
    ...
    def test_suffix_match_component_boundaries(self):
        ...
        assert matches_qualified_pattern("app::ui::View", "ui::View") is True
        assert matches_qualified_pattern("legacy::ui::View", "ui::View") is True
        assert matches_qualified_pattern("app::core::ui::View", "ui::View") is True
        assert (
            matches_qualified_pattern("app::core::ui::View", "core::ui::View") is True
        )

        # Should NOT match: wrong component boundaries
        assert matches_qualified_pattern("myui::View", "ui::View") is False
        assert matches_qualified_pattern("app::myui::View", "ui::View") is False

        # Should NOT match: pattern longer than name
        assert matches_qualified_pattern("View", "app::View") is False
        assert matches_qualified_pattern("ui::View", "app::ui::View") is False
    ...
    def test_regex_patterns(self):
        ...
        assert matches_qualified_pattern("app::core::View", "app::.*::View") is True
        assert matches_qualified_pattern("app::ui::View", "app::.*::View") is True
        assert (
            matches_qualified_pattern("app::View", "app::.*::View") is False
        )  # no middle component

        # Regex without :: (matches anywhere)
        assert matches_qualified_pattern("View", ".*View.*") is True
        assert matches_qualified_pattern("ViewManager", ".*View.*") is True
        assert matches_qualified_pattern("app::View", ".*View.*") is True

        # Regex anchored patterns
        assert matches_qualified_pattern("app::View", "app::View.*") is True
        assert matches_qualified_pattern("app::ViewManager", "app::View.*") is True
        assert matches_qualified_pattern("legacy::View", "app::View.*") is False
    ...
    def test_regex_invalid_patterns(self):
        ...
        assert matches_qualified_pattern("app::View", "[invalid(regex") is False
        assert matches_qualified_pattern("app::View", "(?P<invalid") is False

    def test_empty_pattern_matches_all(self):
        """Empty pattern should match everything."""
        assert matches_qualified_pattern("View", "") is True
        assert matches_qualified_pattern("app::View", "") is True
        assert matches_qualified_pattern("ns1::ns2::View", "") is True
    ...
    def test_multi_component_suffix_patterns(self):
        ...
        assert (
            matches_qualified_pattern("legacy::core::ui::View", "core::ui::View")
            is True
        )
        assert matches_qualified_pattern("app::ui::View", "core::ui::View") is False

        # 2-component pattern
        assert matches_qualified_pattern("app::ui::View", "ui::View") is True
        assert matches_qualified_pattern("app::View", "ui::View") is False
    ...
    def test_edge_cases(self):
        ...
        assert matches_qualified_pattern("app::ui::View", "app::ui::View") is True

        # Pattern equals qualified name (unqualified mode)
        assert matches_qualified_pattern("View", "View") is True

        # Empty qualified name (shouldn't happen in practice, but handle gracefully)
        assert matches_qualified_pattern("", "") is True
        assert matches_qualified_pattern("", "View") is False
