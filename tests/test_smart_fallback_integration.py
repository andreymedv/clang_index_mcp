...
def _create_symbol(
    name, kind="class", file="test.h", line=1, qualified_name=None, namespace="", is_project=True
):
...
class TestSmartFallbackWithSearchEngine(unittest.TestCase):
    ...
    def setUp(self):
        self.class_index = {
            ...
            "Widget": [
                _create_symbol(
                    "Widget", qualified_name="ui::Widget", namespace="ui", file="Widget.h", line=10
                ),
            ],
            "ConsoleReporter": [
                _create_symbol(
                    "ConsoleReporter",
                    qualified_name="test::ConsoleReporter",
                    namespace="test",
                    file="reporter.h",
                    line=20,
                ),
            ...
            "CompactReporter": [
                _create_symbol(
                    "CompactReporter",
                    qualified_name="test::CompactReporter",
                    namespace="test",
                    file="reporter.h",
                    line=40,
                ),
        ...
        self.file_index = {
            ...
            "reporter.h": (
                [self.class_index["ConsoleReporter"][0]] + [self.class_index["CompactReporter"][0]]
            ),
    ...
    def _search_with_fallback(
        self, pattern, tool_name="search_classes", file_name=None, namespace=None, class_name=None
    ):
    ...
    def test_qualified_name_wrong_namespace(self):
        ...
        self.assertTrue(
            any(alt["qualified_name"] == "app::Handler" for alt in fallback.alternatives)
        )

    def test_regex_with_dollar_anchor(self):
        """When regex uses $ anchor, suggest fullmatch-compatible version."""
        results, fallback = self._search_with_fallback("Reporter$", tool_name="search_classes")
        self.assertEqual(results, [])
        self.assertIsNotNone(fallback)
    ...
    def test_no_fallback_when_results_found(self):
        ...
        results, fallback = self._search_with_fallback("Handler", tool_name="search_classes")
