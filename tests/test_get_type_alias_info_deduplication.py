...
class TestAliasDeduplication:
    ...
    def test_namespaced_alias_deduplication(self, temp_project_dir):
        ...
        (temp_project_dir / "src" / "test.cpp").write_text("""
            namespace Outer {
                class TextBuffer { };
                using TextBufferAlias = TextBuffer;
            }
            """)
        ...
        result_canonical = analyzer.get_type_alias_info("Outer::TextBuffer")
        assert result_canonical["canonical_type"] == "Outer::TextBuffer"
        ...
        assert result_canonical["aliases"][0]["qualified_name"] == "Outer::TextBufferAlias"

        # Case 2: Query by alias name
        result_alias = analyzer.get_type_alias_info("Outer::TextBufferAlias")
        assert result_alias["canonical_type"] == "Outer::TextBuffer"
        ...
        assert result_alias["aliases"][0]["qualified_name"] == "Outer::TextBufferAlias"
