...
class TestSearchClassesToGetClassInfo:
    ...
    def test_qualified_name_from_search_works_in_get_info(self, namespaced_project):
        ...
        base_result = next(
            r for r in search_results if r["qualified_name"].split("::")[-1] == "Base"
        )
...
class TestSearchClassesToGetHierarchy:
    ...
    def test_qualified_name_from_search_works_in_hierarchy(self, namespaced_project):
        ...
        derived_results = [
            r for r in search_results if r["qualified_name"].split("::")[-1] == "Derived"
        ]
        ...
        assert hierarchy is not None, f"get_class_hierarchy failed for '{qualified_name}'"
    ...
    def test_hierarchy_returns_qualified_base_classes(self, namespaced_project):
        ...
        search_results = analyzer.search_classes("Derived")
        derived = next(
            (r for r in search_results if r["qualified_name"].split("::")[-1] == "Derived"), None
        )
...
class TestSearchClassesToGetDerived:
    ...
    def test_qualified_name_from_search_works_in_derived(self, namespaced_project):
        ...
        assert derived is not None, f"get_derived_classes failed for '{qualified_name}'"

        # Should find Derived class
        derived_names = [d["qualified_name"].split("::")[-1] for d in derived]
        assert any(
            "Derived" in name for name in derived_names
...
class TestTemplateWorkflows:
    ...
    def test_search_template_to_get_derived(self, template_project):
        ...
        container_results = [
            r
            for r in search_results
            if r["qualified_name"].split("::")[-1] == "Container"
            or r["qualified_name"].endswith("::Container")
        ]
...
class TestCompleteChains:
    ...
    def test_full_chain_search_info_hierarchy_derived(self, namespaced_project):
        ...
        search_results = analyzer.search_classes("Base")
        base = next(
            (r for r in search_results if r["qualified_name"].split("::")[-1] == "Base"), None
        )
    ...
    def test_method_lookup_from_class_info(self, namespaced_project):
        ...
        helper = next(
            (r for r in search_results if r["qualified_name"].split("::")[-1] == "Helper"), None
        )
...
class TestGetFilesContainingSymbolQualifiedNames:
    ...
    def test_fully_qualified_method_returns_file(self, namespaced_project):
        ...
        analyzer = namespaced_project
        result = asyncio.run(analyzer.get_files_containing_symbol("outer::Helper::helperMethod"))
        assert result["files"], "Fully qualified 'outer::Helper::helperMethod' must find files"
    ...
    def test_partially_qualified_class_returns_file(self, namespaced_project):
        ...
        assert result["files"], "Partially qualified class name 'inner::Base' must find files"

    def test_fully_qualified_class_returns_file(self, namespaced_project):
        """outer::inner::Derived should find the file."""
        analyzer = namespaced_project
        result = asyncio.run(
            analyzer.get_files_containing_symbol("outer::inner::Derived", symbol_kind="class")
        )
        assert result["files"], "Fully qualified class name 'outer::inner::Derived' must find files"

    def test_partially_qualified_free_function_returns_file(self, namespaced_project):
        """outer::outerFunction should find the file."""
        analyzer = namespaced_project
        result = asyncio.run(
            analyzer.get_files_containing_symbol("outer::outerFunction", symbol_kind="function")
        )
        assert result[
            "files"
    ...
    def test_nonexistent_qualified_name_returns_empty(self, namespaced_project):
        ...
        analyzer = namespaced_project
        result = asyncio.run(analyzer.get_files_containing_symbol("NoSuchClass::noSuchMethod"))
