...
class TestMaintenanceMethods(unittest.TestCase):
    ...
    def test_vacuum_after_deletions(self):
        ...
        self.assertLessEqual(
            size_after, size_before * 1.1, "VACUUM should not significantly increase size"
        )
    ...
    def test_auto_maintenance_small_db(self):
        ...
        results = self.backend.auto_maintenance(vacuum_threshold_mb=100.0, vacuum_min_waste_mb=10.0)
        ...
        self.assertIsNotNone(
            results["vacuum_skipped_reason"], "Should have reason for skipping VACUUM"
        )
...
class TestHealthCheckMethods(unittest.TestCase):
    ...
    def test_get_health_status(self):
        ...
        self.assertTrue(health["checks"]["integrity"]["passed"], "Integrity check should pass")

        # Verify size check
        self.assertIn("size", health["checks"], "Should have size check")
        self.assertGreater(health["checks"]["size"]["db_size_mb"], 0, "Should report database size")

        # Verify FTS5 check
        self.assertIn("fts_index", health["checks"], "Should have FTS5 check")
        self.assertEqual(health["checks"]["fts_index"]["status"], "ok", "FTS5 should be healthy")

        # Verify WAL mode check
        self.assertIn("wal_mode", health["checks"], "Should have WAL mode check")
        self.assertEqual(
            health["checks"]["wal_mode"]["journal_mode"], "wal", "Should be in WAL mode"
        )
    ...
    def test_get_health_status_with_warnings(self):
        ...
        self.assertEqual(len(health["errors"]), 0, f"Should have no errors: {health['errors']}")
    ...
    def test_get_table_sizes(self):
        ...
        # Verify row counts
        self.assertEqual(tables["symbols"]["row_count"], 100, "Symbols table should have 100 rows")
        self.assertEqual(tables["symbols"]["status"], "ok", "Symbols table should be ok")
...
class TestCacheStatsMethods(unittest.TestCase):
    ...
    def test_get_cache_stats(self):
        ...
        self.assertEqual(stats["file_stats"]["total_files"], 10, "Should have 10 files")
        self.assertGreater(
            stats["file_stats"]["avg_symbols_per_file"], 0, "Should have average symbols per file"
        )
    ...
    def test_monitor_performance_search(self):
        ...
        self.assertGreater(metrics["fts_search_ms"], 0, "FTS search should take some time")
        self.assertGreater(metrics["like_search_ms"], 0, "LIKE search should take some time")
    ...
    def test_monitor_performance_load(self):
        ...
        self.assertGreater(metrics["load_by_usr_ms"], 0, "Load should take some time")

        # Time should be very fast (< 10ms for single lookup)
        self.assertLess(metrics["load_by_usr_ms"], 10, "Load by USR should be very fast")
    ...
    def test_monitor_performance_write(self):
        ...
        self.assertGreater(metrics["write_symbol_ms"], 0, "Write should take some time")

        # Time should be reasonable (< 5ms for single write)
        self.assertLess(metrics["write_symbol_ms"], 5, "Write should be fast")
...
class TestMaintenanceIntegration(unittest.TestCase):
    ...
    def test_maintenance_sequence(self):
        ...
        self.assertIn(
            health_before["status"], ["healthy", "warning"], "Should be healthy before maintenance"
        )
        ...
        self.assertIn(
            health_after["status"], ["healthy", "warning"], "Should be healthy after maintenance"
        )
    ...
    def test_maintenance_does_not_corrupt(self):
        ...
        # Verify counts match
        self.assertEqual(
            stats_before["total_symbols"],
            stats_after["total_symbols"],
            "Symbol count should be unchanged",
        )
        self.assertEqual(
            stats_before["by_kind"],
            stats_after["by_kind"],
