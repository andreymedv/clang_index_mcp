...
class TestBasicInheritance:
    ...
    def test_case1_single_inheritance(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "SingleDerived", "inheritance_test::SingleDerived")
    ...
    def test_case2_multiple_inheritance_2_bases(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "MultiDerived2", "inheritance_test::MultiDerived2")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case3_multiple_inheritance_3_bases(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "MultiDerived3", "inheritance_test::MultiDerived3")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
        assert "MultiBaseA" in bases
    ...
    def test_case5_protected_inheritance(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "ProtectedDerived", "inheritance_test::ProtectedDerived")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
        assert "SingleBase" in bases
    ...
    def test_case6_private_inheritance(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "PrivateDerived", "inheritance_test::PrivateDerived")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
        assert "SingleBase" in bases
    ...
    def test_case7_mixed_access_specifiers(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "MixedAccessDerived", "inheritance_test::MixedAccessDerived"
        )
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case8_struct_from_class(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "StructFromClass", "inheritance_test::StructFromClass")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case8_class_from_struct(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "ClassFromStruct", "inheritance_test::ClassFromStruct")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
        assert "SingleBase" in bases
...
class TestVirtualInheritance:
    ...
    def test_case9_diamond_non_virtual(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "DiamondBottomNV", "inheritance_test::DiamondBottomNV")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case10_diamond_virtual(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "DiamondBottomV", "inheritance_test::DiamondBottomV")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case10_diamond_virtual_bases_have_diamond_top(self, analyzer):
        ...
        for name in ["DiamondLeftV", "DiamondRightV"]:
            info = get_info_or_skip(analyzer, name, f"inheritance_test::{name}")
            assert info is not None
            bases = base_names(info.get("base_classes", []))
    ...
    def test_case11_mixed_virtual_nonvirtual(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "MixedVDerived", "inheritance_test::MixedVDerived")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case12_deep_diamond(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "DeepDiamondBottom", "inheritance_test::DeepDiamondBottom"
        )
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case13_virtual_with_overrides(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "VOverrideBottom", "inheritance_test::VOverrideBottom")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_diamond_hierarchy(self, analyzer):
        ...
        hierarchy = analyzer.get_class_hierarchy("inheritance_test::DiamondBottomV")
        assert hierarchy is not None
        assert "error" not in hierarchy
        qname = hierarchy.get("queried_class")
        node = hierarchy["classes"].get(qname, {})
...
class TestTemplateInheritance:
    ...
    def test_case18_non_template_from_template(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "IntContainer", "inheritance_test::IntContainer")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        # Base should be GenericContainer (possibly with template args)
        assert any(
            "GenericContainer" in b for b in bases
    ...
    def test_case18_double_container(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "DoubleContainer", "inheritance_test::DoubleContainer")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
    ...
    def test_case19_nested_template(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "NestedTemplateChild", "inheritance_test::NestedTemplateChild"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "OuterWrapper" in b for b in bases
    ...
    def test_case20_concrete_from_template(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "ConcreteFromTemplate", "inheritance_test::ConcreteFromTemplate"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "TemplateFromTemplate" in b for b in bases
    ...
    def test_get_derived_generic_container(self, analyzer):
        ...
        derived_names = [d["qualified_name"].split("::")[-1] for d in derived]
        assert (
            "IntContainer" in derived_names or "DoubleContainer" in derived_names
...
class TestCRTPPatterns:
    ...
    def test_case21_basic_crtp(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "CRTPConcrete", "inheritance_test::CRTPConcrete")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "CRTPBase" in b for b in bases
    ...
    def test_case22_crtp_with_intermediate(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "CRTPBottom", "inheritance_test::CRTPBottom")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case23_crtp_with_extra_bases(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "CRTPExtraConcrete", "inheritance_test::CRTPExtraConcrete"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "CRTPWithExtra" in b for b in bases
    ...
    def test_case24_multi_level_crtp(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "CRTPChainEnd", "inheritance_test::CRTPChainEnd")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "CRTPLevel2" in b for b in bases
    ...
    def test_case25_crtp_pure_interface(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "CRTPPureConcrete", "inheritance_test::CRTPPureConcrete")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "CRTPPureInterface" in b for b in bases
...
class TestMixinPatterns:
    ...
    def test_case26_single_mixin(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "LoggedTarget", "inheritance_test::LoggedTarget")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "LoggingMixin" in b for b in bases
    ...
    def test_case27_stacked_mixins(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "FullyMixedTarget", "inheritance_test::FullyMixedTarget")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
    ...
    def test_case28_policy_based_design(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "DefaultPolicyHost", "inheritance_test::DefaultPolicyHost"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "PolicyHost" in b for b in bases
    ...
    def test_case28_custom_policy(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "CustomPolicyHost", "inheritance_test::CustomPolicyHost")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
    ...
    def test_case29_mixin_with_fixed_base(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "MixinFixedTarget", "inheritance_test::MixinFixedTarget")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "MixinWithFixed" in b for b in bases
    ...
    def test_case30_crtp_mixin_combo(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "CRTPMixinConcrete", "inheritance_test::CRTPMixinConcrete"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "CRTPMixin" in b for b in bases
...
class TestNameCollision:
    ...
    def test_case31_real_derived_from_concrete_base(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "RealDerivedFromBase", "inheritance_test::RealDerivedFromBase"
        )
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case33_concrete_from_other_namespace(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "ConcreteWidgetChild", "inheritance_test::ConcreteWidgetChild"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "Widget" in b for b in bases
...
class TestForwardDeclaration:
    ...
    def test_case35_fwd_decl_definition_has_base(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "FwdDerived", "inheritance_test::FwdDerived")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
        assert (
    ...
    def test_case37_multiple_fwd_decls(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "MultiFwdDerived", "inheritance_test::MultiFwdDerived")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
        assert (
            "MultiFwdBase" in bases
...
class TestExternTemplate:
    ...
    def test_case41_extern_mixed_fixed_and_param_bases(self, analyzer):
        ...
        for spec in specs:
            bases = base_names(spec.get("base_classes", []))
            assert "ExternBase" in bases, f"Should have ExternBase from T param, got {bases}"
            assert (
                "ExternMixedFixed" in bases
...
class TestSpecialization:
    ...
    def test_case42_primary_has_spec_base_a(self, analyzer):
        ...
        primary = None
        for r in results:
            if r["qualified_name"].split("::")[-1] == "SpecPrimary" and r.get(
                "template_kind"
...
class TestNamespaceAmbiguity:
    ...
    def test_case48_inherits_from_ns1(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "InheritsFromNs1", "inheritance_test::InheritsFromNs1")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
    ...
    def test_case48_inherits_from_ns2(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "InheritsFromNs2", "inheritance_test::InheritsFromNs2")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any("Base" in b for b in bases)
    ...
    def test_case49_using_declaration_inheritance(self, analyzer):
        ...
        info = analyzer.get_class_info("inheritance_test::target_ns::InheritsImported")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "ImportedBase" in b for b in bases
    ...
    def test_case50_nested_namespace_item(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "DerivedFromAB", "inheritance_test::DerivedFromAB")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "Item" in b for b in bases
    ...
    def test_case50_derived_from_cd(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "DerivedFromCD", "inheritance_test::DerivedFromCD")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
    ...
    def test_case53_qualified_vs_unqualified(self, analyzer):
        ...
        for name, qname in [
            ...
            bases = info.get("base_classes", [])
            assert any("Base" in b for b in bases), f"{name} should inherit from Base, got {bases}"
...
class TestAliasInheritance:
    ...
    def test_case54_simple_type_alias(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "DerivedFromAlias", "inheritance_test::DerivedFromAlias")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
    ...
    def test_case55_template_alias(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "DerivedFromTemplateAlias", "inheritance_test::DerivedFromTemplateAlias"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
    ...
    def test_case56_alias_to_template_instantiation(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "DerivedFromIntStore", "inheritance_test::DerivedFromIntStore"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
    ...
    def test_case57_cross_namespace_alias(self, analyzer):
        ...
        info = analyzer.get_class_info("inheritance_test::alias_target::DerivedFromCrossNsAlias")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
    ...
    def test_case58_chain_of_aliases(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "DerivedFromChainAlias", "inheritance_test::DerivedFromChainAlias"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
    ...
    def test_case59_template_alias_with_policy(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "DerivedFromPolicyAlias", "inheritance_test::DerivedFromPolicyAlias"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
...
class TestAdvancedPatterns:
    ...
    def test_case60_variadic_concrete(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "VariadicConcrete", "inheritance_test::VariadicConcrete")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "VariadicInherit" in b for b in bases
    ...
    def test_case61_outer_class_inherits(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "OuterClass", "inheritance_test::OuterClass")
        assert info is not None
        assert "error" not in info
        bases = base_names(info.get("base_classes", []))
    ...
    def test_case62_uses_default_template_param(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "UsesDefault", "inheritance_test::UsesDefault")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "DefaultParamInherit" in b for b in bases
    ...
    def test_case62_overrides_default(self, analyzer):
        ...
        info = get_info_or_skip(analyzer, "OverridesDefault", "inheritance_test::OverridesDefault")
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
    ...
    def test_case64_same_template_different_args(self, analyzer):
        ...
        for name in ["FromScopedInt", "FromScopedDouble"]:
            ...
            bases = info.get("base_classes", [])
            assert any(
                "ScopedTemplate" in b for b in bases
    ...
    def test_case65_inherits_nested_type(self, analyzer):
        ...
        info = get_info_or_skip(
            analyzer, "ConcreteNestedInherit", "inheritance_test::ConcreteNestedInherit"
        )
        assert info is not None
        assert "error" not in info
        bases = info.get("base_classes", [])
        assert any(
            "InheritsNestedType" in b for b in bases
...
class TestCrossToolWorkflows:
    ...
    def test_hierarchy_base_count_matches_info(self, analyzer):
        ...
        assert len(info_bases) == len(hier_bases), (
            f"Base count mismatch: info has {len(info_bases)}, " f"hierarchy has {len(hier_bases)}"
        )
...
class TestHierarchyQualifiedNames:
    ...
    def test_get_derived_classes_has_qualified_name(self, analyzer):
        ...
        for d in derived:
            assert "qualified_name" in d, f"derived class entry missing 'qualified_name': {d}"
    ...
    def test_hierarchy_nodes_have_qualified_name(self, analyzer):
        ...
        for key, node in hierarchy["classes"].items():
            if node.get("is_dependent_type") or node.get("is_unresolved"):
                continue
            assert "qualified_name" in node, f"Node missing 'qualified_name': {node}"
            assert (
                node["qualified_name"] == key
    ...
    def test_hierarchy_multi_level_qualified_names(self, analyzer):
        ...
        for key, node in hierarchy["classes"].items():
            if node.get("is_dependent_type") or node.get("is_unresolved"):
                continue
            assert "qualified_name" in node, f"Missing qualified_name in node: {node}"
    ...
    def test_derived_classes_qualified_name_usable_with_get_class_info(self, analyzer):
        ...
        for d in derived:
            qname = d["qualified_name"]
            info = analyzer.get_class_info(qname)
            assert info is not None, f"get_class_info('{qname}') returned None"
            assert (
                "error" not in info
...
class TestGetClassInfoDerivedClasses:
    ...
    def test_get_class_info_has_derived_classes_key(self, analyzer):
        ...
        assert info is not None and "error" not in info
        assert "derived_classes" in info, "get_class_info result missing 'derived_classes' key"
    ...
    def test_get_class_info_derived_classes_includes_direct_child(self, analyzer):
        ...
        derived_names = [d["qualified_name"].split("::")[-1] for d in derived]
        assert (
            "SingleDerived" in derived_names
    ...
    def test_get_class_info_derived_classes_is_list(self, analyzer):
        ...
        assert info is not None and "error" not in info
        assert isinstance(info["derived_classes"], list), "derived_classes should be a list"
    ...
    def test_get_class_info_derived_classes_have_qualified_name(self, analyzer):
        ...
        for d in info["derived_classes"]:
            assert "qualified_name" in d, f"derived_classes entry missing 'qualified_name': {d}"
...
class TestHierarchyCaps:
    ...
    def test_max_nodes_cap_triggers_truncation(self, analyzer):
        ...
        hierarchy = analyzer.get_class_hierarchy("inheritance_test::DeepA", max_nodes=2)
        assert "error" not in hierarchy
        assert (
            hierarchy.get("truncated") is True
        ), "Expected truncated=True when max_nodes=2 on a 5-class chain"
        assert hierarchy.get("nodes_returned") == len(
            hierarchy["classes"]
        ), "nodes_returned must equal actual class count"
        assert (
            len(hierarchy["classes"]) <= 2
    ...
    def test_max_nodes_cap_includes_queried_class(self, analyzer):
        ...
        hierarchy = analyzer.get_class_hierarchy("inheritance_test::DeepC", max_nodes=1)
        assert "error" not in hierarchy
        assert (
            "inheritance_test::DeepC" in hierarchy["classes"]
    ...
    def test_max_nodes_none_disables_cap(self, analyzer):
        ...
        hierarchy = analyzer.get_class_hierarchy("inheritance_test::DeepA", max_nodes=None)
        assert "error" not in hierarchy
        assert (
            "truncated" not in hierarchy
    ...
    def test_max_depth_zero_returns_only_queried_class(self, analyzer):
        ...
        hierarchy = analyzer.get_class_hierarchy("inheritance_test::DeepC", max_depth=0)
        assert "error" not in hierarchy
        assert (
            "inheritance_test::DeepC" in hierarchy["classes"]
        ), "Queried class must be present with max_depth=0"
        assert (
            len(hierarchy["classes"]) == 1
    ...
    def test_max_depth_covers_full_chain(self, analyzer):
        ...
        hierarchy = analyzer.get_class_hierarchy("inheritance_test::DeepA", max_depth=10)
        assert "error" not in hierarchy
        assert (
            "truncated" not in hierarchy
    ...
    def test_nodes_returned_matches_class_count(self, analyzer):
        ...
        hierarchy = analyzer.get_class_hierarchy("inheritance_test::DeepA", max_nodes=3)
