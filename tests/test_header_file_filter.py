...
""")
        ...
        assert (
            len(myclass_results) == 2
        ), f"Should find exactly 2 classes in MyClass.h, found {len(myclass_results)}"
        ...
        for result in myclass_results:
            ...
            assert _res_loc["file"].endswith(
                "MyClass.h"
            ), f"Result file should be MyClass.h, got {_res_loc['file']}"
...
""")
        ...
        assert (
            "add" in functions_h_names or "subtract" in functions_h_names
        ...
        for result in functions_h_results:
            ...
            assert _decl_file.endswith("functions.h") or _def_file.endswith(
                "functions.h"
...
""")
        ...
        results = analyzer.search_functions(
            "commonMethod", class_name="ClassA", file_name="Multi.h"
        )
...
""")
        ...
        for result in results:
            ...
            assert _res_loc["file"].endswith(
                "Target.h"
            ), f"Found unexpected file: {_res_loc['file']}"
