...
async def list_tools() -> List[Tool]:
    return [
                ...
                "properties": {
                    ...
                    "namespace": {
                        ...
                        "description": (
                            ...
                            "'ItemBuilder' matches 'Outer::ItemBuilder', 'app' matches 'myapp::app', "
        ...
        Tool(
            name="get_class_info",
            description="Get comprehensive information about a specific class: methods with signatures (all access levels), base classes, file location, line ranges, documentation, and virtual/abstract indicators. **Note**: Member variables/fields (members) are not currently indexed and will be an empty list. **Use this when**: user wants to see class methods or API. **Requires exact class name** - if you don't know exact name, use search_classes first.\n\n**IMPORTANT:** If called during indexing, results will be incomplete. Check response metadata 'status' field. Use 'wait_for_indexing' first if you need guaranteed complete results.\n\nReturns: name, kind, file, line, base_classes, methods (sorted by line, each with signature, access, line ranges, is_template_specialization for method templates, is_virtual, is_pure_virtual, is_const, is_static, is_definition, brief, and doc_comment), members (currently empty), is_project, start_line, end_line (class body range), header_file (if declared in header), header_start_line, header_end_line, brief (class documentation first line or null), doc_comment (class full documentation up to 4000 chars or null). Use is_pure_virtual to identify abstract interfaces - if a class has pure virtual methods, implementations should be in derived classes, not with the same class qualifier. Documentation extracted from Doxygen (///, /** */), JavaDoc, and Qt-style (/*!) comments. Returns plain text error 'Class <name> not found' if not found. Returns first match if multiple classes have same name.",
            inputSchema={
                "type": "object",
                "properties": {
                    "class_name": {
