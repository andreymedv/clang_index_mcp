...
class CompileCommandsManager:
    ...
    def _get_bundled_cxx_stdlib_path(self) -> Optional[str]:
    ...
    def _detect_system_c_headers_dir(self) -> Optional[str]:
        ...
        if self.clang_resource_dir:
            ...
            clang_lib = os.path.dirname(self.clang_resource_dir)  # .../clang/X.Y.Z
        ...
        for pattern in sdk_patterns:
            matches = sorted(glob_module.glob(pattern), reverse=True)
            if matches:
                return matches[0]
    ...
    def _detect_cxx_stdlib_path(self, arguments: List[str]) -> Optional[str]:
        ...
        if stdlib:
            ...
            if stdlib == "libc++":
                path = self._get_libcxx_path(sysroot)
                if path:
                    return path
            elif stdlib == "libstdc++":
                path = self._get_libstdcxx_path(sysroot)
                if path:
                    return path

        # Fallback: check bundled includes shipped with the project
        return self._get_bundled_cxx_stdlib_path()
    ...
    def _add_builtin_includes(self, arguments: List[str]) -> List[str]:
        ...
        if cxx_stdlib_path and not self._is_path_in_args(cxx_stdlib_path, result):
            ...
            system_c_dir = self._detect_system_c_headers_dir()
            if system_c_dir and not self._is_path_in_args(system_c_dir, result):
                insert_pos = self._insert_system_include(result, insert_pos, system_c_dir)
